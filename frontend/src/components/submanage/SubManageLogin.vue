<template>
  <div class="cover">
    <div class="modal">
      ...
      <h1>分站管理系统登录</h1>
      <el-form :model="loginForm" :rules="loginFormRules" label-width="0px" class="loginForm" ref="loginForm">
            ...
            this.$store.dispatch('systemManagerLogin', this.loginForm)
              .then((error)=>{
                if(error && error.invalidKey){
                  this.errors[error.invalidKey] = error.errorInfo;
                }else{
                  this.$router.push("/deliverPlans");
                }
              })
          } else {
            return false;
